...
public class AuthController {
    ...
    public Result<String> getToken(@PathVariable String name) {
        if (StringUtils.isEmpty(name)) {
            return new Result<>(ResponseCode.Etc, "用户名不能为空!", null);
        }
        String token = JwtUtil.getToken(name);
        return new Result<>(ResponseCode.Success, "success", token);
    }
}
